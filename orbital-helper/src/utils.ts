...
export const prettyDuration = ({startMillis, endMillis=Date.now()}: {startMillis: number, endMillis?: number}) => {
    ...
    const daysPart = durationMillis>=millisInDay ?
        `${Math.floor(durationMillis / millisInDay).toLocaleString()} days ` :
        ''
    const hoursPart = durationMillis>=millisInHour ?
        `${Math.floor(durationMillis / millisInHour) % 24} hrs ` :
        ''
    const minutesPart = durationMillis>=millisInMinute ?
        `${Math.floor(durationMillis / millisInMinute) % 60} min ` :
        ''
    const secondsPart = `${roundToHundredths((durationMillis  / 1_000) % 60)} sec`
