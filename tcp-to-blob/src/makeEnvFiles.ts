...
const makeEnvFiles = ({templateNames, outDir}: MakeEnvFilesParams = {}) => {
    ...
    console.log(`Making env files: ${JSON.stringify({outDir, templateNames}, null, 2)}`)
...
if(require.main === module) {
    program
        .option('-t, --templates <string>', 'e.g. "./a-template.yaml,b-template.json"')
    ...
    try {
        ...
        throw new Error(`-t/-templates- argument should be a comma separated list: "${templateNamesString}"`)
    }
    if(!templateNames) {
